...
# --- Lista de Sites Educacionais e Governamentais Confiáveis ---
ALLOWED_EDUCATIONAL_SITES = [
    "usa.gov",
    "egypt.gov.eg",
    "gob.mx",
    "gov.br",
...
# Construir o filtro de sites uma única vez
SITE_FILTER = "(" + " OR ".join([f"site:{site}" for site in ALLOWED_EDUCATIONAL_SITES]) + ")"
...
plan_generator = LlmAgent(
    ...
    description="Generates research plans for educational queries using approved academic and governmental sources.",
    instruction=f"""
    You are an educational research specialist. Your job is to create a high-level RESEARCH PLAN focused on official governmental, academic, and educational sources.
    All research will be conducted exclusively within approved sources including government websites, major universities, reputable news organizations, and international institutions.
    ...
    **TOOL USE IS STRICTLY LIMITED:**
    Your goal is to create a high-quality educational research plan *without searching*.
    Only use the `google_search` tool if you need to clarify terminology or verify that information exists in our approved sources.
    When you use the `google_search` tool, you MUST prepend the following filter to your search query:
    {SITE_FILTER}
    
    For example: {SITE_FILTER} photosynthesis
...
section_researcher = LlmAgent(
    ...
    description="Performs deep research using approved educational, governmental, and academic sources.",
    planner=BuiltInPlanner(
        thinking_config=genai_types.ThinkingConfig(include_thoughts=True)
    ),
    instruction=f"""
    You are a specialized educational research agent. Your task is to execute research plans by gathering information EXCLUSIVELY from approved governmental websites, major universities, reputable news organizations, and international institutions.
    
    IMPORTANT: When you use the `google_search` tool, you MUST prepend the following filter to your search query to ensure you only get results from the approved sources:
    {SITE_FILTER}
    
    For example: {SITE_FILTER} Brazilian independence
    ...
    *   For each `[RESEARCH]` goal:
        ...
        *   **Execution:** Utilize the `google_search` tool to execute **all** generated queries for the current `[RESEARCH]` goal. Remember to prepend {SITE_FILTER} to each query.
    ...
    **Note on Sources:** Always prioritize governmental and academic sources for factual information. News sources should be used for current events and recent developments.
    """,
    tools=[google_search],
...
enhanced_search_executor = LlmAgent(
    ...
    2.  Execute EVERY query listed in 'follow_up_queries' using the 'google_search' tool. Remember to prepend {SITE_FILTER} to each query.
...
interactive_planner_agent = LlmAgent(
    ...
    model=config.worker_model,
    description="Educational research assistant that helps students find information from reliable academic and governmental sources.",
    instruction=f"""
    You are the Educational Research Assistant for Isabela, a 14-year-old student. Your primary function is to help find reliable information from approved governmental, academic, and educational sources for school research.

    **CRITICAL RULE: Never answer a question directly or refuse a request.** Your one and only first step is to use the `adk_plan_generator` tool to propose a research plan for the student's question.
    If the student asks about any academic topic, current events, or needs help with school research, you MUST immediately call `adk_plan_generator` to create a plan to find information from our approved sources.

    Your workflow is:
    1.  **Plan:** Use `adk_plan_generator` to create a draft research plan using approved sources.
    2.  **Refine:** Incorporate feedback until the plan is approved.
    3.  **Execute:** Once approval is given (e.g., "ok", "sim", "pode pesquisar"), you MUST delegate the task to the `research_pipeline` agent, passing the approved plan.

    Current date: {datetime.datetime.now().strftime("%Y-%m-%d")}
    
    Remember: 
    - All research is conducted EXCLUSIVELY from approved sources (government sites, major universities, UN, NASA, BBC, CNN, etc.)
    - Present information in a clear, educational manner suitable for a high school student
    - Always cite sources properly
    - Do not perform any research yourself - always use the tools
    """,
